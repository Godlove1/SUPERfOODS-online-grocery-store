...
$promop = mysqli_real_escape_string($conn, $_POST['pprice']);
...
if (!empty($_FILES['images']['name'][0])) {
    ...
    for($i = 0; $i < $countfiles; $i++) {
        ...
        if ($file_type != 'image/jpeg' && $file_type != 'image/png' && $file_type != 'image/gif'  && $file_type != 'image/webp'  && $file_type != 'image/jfif') {
...
echo "<option value='0'>Category Not Available.</option>";
         ...
         <div class="">
        <label class="block font-medium mb-1" >Measurement </label>
        <select name="measurement" class="h-10 px-3 mb-2 text-base bg-white  border border-slate-400 rounded-lg focus:outline-none">
          <option value="">(no unit)</option>
        <?php
//1. CReate SQL to get all active categories from database
$sql = "SELECT * FROM tbl_measurements";
//Execute the Query
$res = mysqli_query($conn, $sql);
//Count Rows
$count = mysqli_num_rows($res);
...
$m_title = $row['m_unit'];
?>
<option <?php if($ms==$m_title){echo "selected";} ?> value="<?php echo $m_title; ?>"><?php echo $m_title; ?></option>
<?php } } else {//CAtegory Not Available
echo "<option value=''>measurement Not Available.</option>";
       ...
       <div class="">
        <label class="block mb-1 font-medium" for="forms-labelOverInputCode">Quantity <span class="text-xs">(enter qty if measurement is selected)</span> </label>
        <input class="w-full h-10 px-3 text-base placeholder-gray-300 border border-slate-400 rounded-lg focus:outline-none placeholder:text-xs" type="text" value="<?php echo $qty ?>" name="qty"/>
      </div>
...
<img src="../assets/images/products/<?php echo $cur_item; ?>" alt="/" width="100px" height="100px" class="rounded ml-2 border-2">
      ...
      <div class="mt-4">
        ...
        <textarea  name="desc" class="w-full h-10 px-3 text-base placeholder-gray-300 border border-slate-400 rounded-lg focus:outline-none"><?php echo $description; ?></textarea>
      </div>
    ...
    <input class="w-1/2 h-10 px-3 mt-6 font-semibold bg-teal-500 cursor-pointer text-white rounded-lg focus:outline-none" type="submit" name="update" value="Update"/>
